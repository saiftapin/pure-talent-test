...
const Listing: React.FC<ListingProps> = ({ apiUrl, limit = 20, renderWith, sortBy }) => {
  ...
  const pageClick = useCallback((page) => {
    setPage(page)
    setSearchParams({search: searchParams.get("search") || "", page: `${page}`})
...
const filterRecords = (
    ...
    if (data && data.length > 0) {
      if (search.length > 0) {
        ...
        records = data.filter(
          (c: any) => c.name.toLowerCase().split(' ').reduce((sum: number, name:string) => sum + (name.indexOf(searchText) === 0 ? 1 : 0), 0) > 0
        );
      } else {
        records = [...data];
      }
